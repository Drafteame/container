...
// try to return a previously created instance of that dependency instead of creating a new instance.
func (c *Container) Get(name string) (any, error) {
	dep, ok := c.deps[name]
	if !ok {
		return dependency.Dependency{}, fmt.Errorf("inject: no provided dependency of name `%s`", name)
	}
